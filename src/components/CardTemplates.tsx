...
const ModernTemplate = forwardRef<HTMLDivElement, TemplateProps>(({ data, scale = 1, onUpdateData, isExporting = false }, ref) => {
  ...
  const [, setIsDraggingAuthors] = useState(false);
  ...
  const handleAuthorsMouseDown = (e: React.MouseEvent) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      ...
      onUpdateData('authorsLayout', { ...initialLayout, x: initialLayout.x + dx, y: initialLayout.y + dy });
    };
    const onMouseUp = () => {
            ...
            <div className="flex flex-col gap-4 items-start justify-start">
              {data.authors.map((author) => (
                <div key={author.id} className="flex items-center gap-4 backdrop-blur-md bg-black/50 rounded-full pr-5 pl-2 py-2 border border-white/20 select-none">
                  <div className="w-12 h-12 rounded-full bg-gray-200 border-2 border-white/40 overflow-hidden shadow-md shrink-0">
                    {author.image ? (
      ...
      {!isFullPortrait && (
        <div className="flex-1 bg-white px-12 py-6 flex items-center justify-between border-t border-gray-100 relative z-10">
          <div className="flex-1 pr-10 flex flex-col justify-center items-start h-full min-w-0">
            ...
            <h3 className="font-serif font-black text-gray-800 italic leading-tight mb-5 whitespace-pre-wrap text-left" style={{ fontSize: `${data.footerFontSize || 48}px` }}>
    ...
    <div ref={ref} className="w-[1080px] h-[720px] bg-[#1e1e1e] shadow-2xl relative overflow-hidden flex flex-col font-mono text-sm" style={{ transform: `scale(${scale})`, transformOrigin: 'top left' }}>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex min-w-0">
          ...
          <div className="w-[45%] flex flex-col bg-white h-full relative">
...
const LivestreamTemplate = forwardRef<HTMLDivElement, TemplateProps>(({ data, scale = 1, onUpdateData, isExporting = false }, ref) => {
  ...
  return (
    ...
    >
      ...
      <div className="px-10 py-6 flex items-center justify-between z-20 relative">
      ...
      <div className="flex-1 flex px-10 pb-10 gap-8 min-h-0 relative z-10">
        <div className="aspect-[16/9] h-full border-[6px] border-[#6366f1] rounded-[32px] bg-transparent relative shrink-0 z-10">
          <div
            ...
            style={{ boxShadow: `0 0 0 2000px ${isDark ? '#1a1a1a' : '#f8f9fa'}` }}
          ></div>
        ...
        <div className="flex-1 flex flex-col min-w-0 relative z-10">
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 flex flex-col justify-start">
              {data.authors && data.authors.length > 0 ? (
                <div
                  className={`${data.authors.length >= 4 ? 'grid grid-cols-2 gap-x-8' : 'flex flex-col'} items-center transition-all duration-300`}
                  style={{
                    ...
                    gap: data.authorsGap !== undefined ? `${data.authorsGap}px` : '24px'
                  }}
                >
                  {data.authors.map((guest) => (
                    <div key={guest.id} className="flex flex-col items-center text-center w-full">
                      <div className={`w-24 h-24 rounded-full overflow-hidden border-[4px] shadow-md mb-3 shrink-0 transition-colors duration-500 ${isDark ? 'border-white/10 bg-white/5' : 'border-gray-100 bg-gray-200'}`}>
                        {guest.image ? (
...
LivestreamTemplate.displayName = 'LivestreamTemplate';
