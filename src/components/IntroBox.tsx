...
const IntroBox: React.FC<IntroBoxProps> = ({ onOpen, onClose, onSelectTemplate, appState }) => {
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, #f0f4f8 0%, #dbeafe 100%)' }}
    >
      ...
      <div
        ...
        style={{ transform: `translateY(${isOpen ? '40px' : '0px'})` }}
      >
        <div
        ...
        >
          ...
          <div
            ...
            style={{ width: boxDepth, left: -boxDepth / 2, transform: 'rotateY(-90deg)', transformOrigin: 'center' }}
          ></div>
          {/* Right Wall */}
          <div
            className="absolute top-0 bottom-0 rounded-sm border border-white/20 bg-gradient-to-b from-white/30 to-white/10 pointer-events-none backdrop-blur-sm"
            style={{ width: boxDepth, right: -boxDepth / 2, transform: 'rotateY(90deg)', transformOrigin: 'center' }}
          ></div>
          {/* Bottom Floor */}
          <div
            className="absolute left-0 right-0 rounded-sm border border-white/20 bg-white/40 pointer-events-none backdrop-blur-md"
            style={{ height: boxDepth, bottom: -boxDepth / 2, transform: 'rotateX(-90deg)', transformOrigin: 'center', boxShadow: 'inset 0 0 50px rgba(255,255,255,0.5)' }}
          ></div>

          {/* Cards Container */}
          <div className="absolute inset-0 flex items-center justify-center preserve-3d" style={{ transform: 'translateZ(0px)' }}>
