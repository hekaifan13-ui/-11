...
const ImageGenModal: React.FC<ImageGenModalProps> = ({ onSelect, onClose, initialPrompt = '' }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError("AI image generation requires backend integration. Please connect Enter Cloud.");
    } finally {
          ...
          {generatedImage ? (
            <div className="flex flex-col items-center w-full h-full relative z-20">
              <div className="flex-1 w-full flex items-center justify-center min-h-0">
                <img src={generatedImage} alt="AI Generated" className="max-w-full max-h-full rounded-lg shadow-2xl object-contain border-4 border-white" />
              </div>
              <div className="mt-6 w-full flex gap-3">
                <button onClick={() => onSelect(generatedImage)} className="flex-1 bg-green-500 hover:bg-green-600 text-white py-2 rounded-lg font-bold shadow-lg flex items-center justify-center gap-2 transition-transform hover:scale-105 cursor-pointer z-20 relative">
                  <Check size={18} /> Use This
                </button>
                <button onClick={handleDownloadImage} className="px-3 bg-white hover:bg-gray-100 text-gray-700 py-2 rounded-lg font-bold shadow-md border border-gray-200 cursor-pointer z-20 relative flex items-center justify-center transition-colors" title="Download Image">
                  <Download size={18} />
                </button>
                <button onClick={() => setGeneratedImage(null)} className="px-4 bg-white hover:bg-gray-100 text-gray-700 py-2 rounded-lg font-bold shadow-md border border-gray-200 cursor-pointer z-20 relative">
                  Discard
                </button>
              </div>
            </div>
            ...
            <div className="text-center text-gray-400 flex flex-col items-center relative z-20">
              {error ? (
                <div className="bg-red-50 text-red-500 p-4 rounded-xl text-sm max-w-xs border border-red-100 mb-4">{error}</div>
              ) : null}
